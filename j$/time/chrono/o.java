...
public final class o extends a implements Serializable {
    ...
    public final b D(int v, int v1, int v2) {
        return q.V(this, v, v1, v2);
    }

    @Override  // j$.time.chrono.a
    public final b G(Map map0, E e0) {
        return (q)super.G(map0, e0);
    }

    @Override  // j$.time.chrono.l
    public final t H(j..time.temporal.a a0) {
        this.R();
        if(a0 instanceof j..time.temporal.a) {
            switch(a0) {
                case 1: {
                    return t.k(1L, this.k, this.l);
                }
                case 2: {
                    return t.j(1L, this.n);
                }
                case 3: {
                    return t.j(1L, 5L);
                }
                case 4: 
                case 5: {
                    return t.j(this.U(0), this.U(this.g.length - 1) - 1);
                }
                case 6: {
                    return t.j(1L, 1L);
                }
    ...
    @Override  // j$.time.chrono.l
    public final List K() {
        return j..com.android.tools.r8.a.k(r.values());
    }

    @Override  // j$.time.chrono.l
    public final boolean N(long v) {
        this.R();
        return v >= ((long)this.U(0)) && v <= ((long)(this.U(this.g.length - 1) - 1)) && this.b0(((int)v), 12) > 354;
    }

    @Override  // j$.time.chrono.l
    public final m O(int v) {
        if(v != 1) {
            throw new j..time.b("invalid Hijrah era");  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        return r.AH;
    }
    ...
    private void R() {
        ...
        if(!this.f) {
            try {
                ...
                int[][] arr2_v = o.a0();
                int v = (int)LocalDate.c0(0x75A, 11, 12).v();
                for(int v2 = 1300; v2 <= 1600; ++v2) {
                    int[] arr_v = arr2_v[v2 - 1300];
                    if(arr_v.length != 12) {
                        throw new IllegalArgumentException("wrong number of months on line: " + Arrays.toString(arr_v) + "; count: " + arr_v.length);
                    }
                    hashMap0.put(v2, arr_v);
                }
                if(!s.equals("Hijrah-umalqura")) {
                ...
                this.j = 0x3CF0;
                this.h = v;
                int[] arr_v1 = this.T(v, hashMap0);
                this.g = arr_v1;
                this.i = arr_v1[0xE1C];
                for(int v1 = 1300; v1 < 1600; ++v1) {
                    int v3 = this.b0(v1, 12);
                    this.m = Math.min(this.m, v3);
                    this.n = Math.max(this.n, v3);
                }
                this.f = true;
                return;
            }
            ...
            throw new j..time.b("Unable to initialize HijrahCalendar: " + s, exception0);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;Ljava/lang/Throwable;)V
        }
    }

    final int S(long v) {
        if(v < ((long)this.U(0)) || v > ((long)(this.U(this.g.length - 1) - 1))) {
            throw new j..time.b("Invalid Hijrah year: " + v);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        return (int)v;
    }

    private int[] T(int v, HashMap hashMap0) {
        int[] arr_v = new int[0xE1D];
        this.k = 0x7FFFFFFF;
        this.l = 0x80000000;
        int v1 = 0;
        for(int v2 = 1300; v2 <= 1600; ++v2) {
            int[] arr_v1 = (int[])hashMap0.get(v2);
            int v3 = 0;
            while(v3 < 12) {
                int v4 = arr_v1[v3];
                arr_v[v1] = v;
                ...
                v += v4;
                this.k = Math.min(this.k, v4);
                this.l = Math.max(this.l, v4);
                ++v3;
                ++v1;
            }
        }
        arr_v[v1] = v;
        if(v1 + 1 != 0xE1D) {
            throw new IllegalStateException("Did not fill epochMonths exactly: ndx = " + (v1 + 1) + " should be " + 0xE1D);
        }
        return arr_v;
    }
    ...
    final int V(int v, int v1) {
        return this.b0(v, v1 - 1);
    }

    final long W(int v, int v1, int v2) {
        this.R();
        if(v1 < 1 || v1 > 12) {
            throw new j..time.b("Invalid Hijrah month: " + v1);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        int v3 = v1 - 1 + (v * 12 - this.j);
        if(v3 < 0 || v3 >= this.g.length) {
            throw new j..time.b("Invalid Hijrah date, year: " + v + ", month: " + v1);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        if(v2 < 1 || v2 > this.Y(v, v1)) {
            throw new j..time.b("Invalid Hijrah day of month: " + v2);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        return (long)(v2 - 1 + this.g[v3]);
    }

    final int[] X(int v) {
        this.R();
        if(v < this.h || v >= this.i) {
            throw new j..time.b("Hijrah date out of range");  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        int v1 = Arrays.binarySearch(this.g, v);
        if(v1 < 0) {
            v1 = -v1 - 2;
        }
        return new int[]{this.U(v1), (this.j + v1) % 12 + 1, v - this.g[v1] + 1};
    }

    final int Y(int v, int v1) {
        int v2 = v1 - 1 + (v * 12 - this.j);
        if(v2 >= 0) {
            int[] arr_v = this.g;
            if(v2 < arr_v.length) {
                return arr_v[v2 + 1] - arr_v[v2];
            }
        }
        throw new j..time.b("Invalid Hijrah date, year: " + v + ", month: " + v1);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
    }
    ...
    @Override  // j$.time.chrono.a
    public final b n() {
        return q.W(this, LocalDate.b0(Clock.b()).u(j..time.temporal.a.EPOCH_DAY));
    }

    // 去混淆评级： 低(20)
    @Override  // j$.time.chrono.l
    public final b o(TemporalAccessor temporalAccessor0) {
        return temporalAccessor0 instanceof q ? ((q)temporalAccessor0) : q.W(this, temporalAccessor0.u(j..time.temporal.a.EPOCH_DAY));
    }
    ...
    public final b w(int v, int v1) {
        q q0 = q.V(this, v, 1, 1);
        if(v1 > q0.L()) {
            throw new j..time.b("Invalid dayOfYear: " + v1);  // 初始化器: Ljava/lang/RuntimeException;-><init>(Ljava/lang/String;)V
        }
        return q0.X(((long)(v1 - 1)));
    }
