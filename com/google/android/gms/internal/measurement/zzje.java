package com.google.android.gms.internal.measurement;

import java.util.Arrays;
import java.util.Collection;
import java.util.RandomAccess;

final class zzje extends zzie implements zzkc, zzlo, RandomAccess {
    private static final double[] zza;
    private double[] zzb;
    private int zzc;

    static {
        double[] arr_f = new double[0];
        zzje.zza = arr_f;
        new zzje(arr_f, 0, false);
    }

    zzje() {
        this(zzje.zza, 0, true);
    }

    private zzje(double[] arr_f, int v, boolean z) {
        super(z);
        this.zzb = arr_f;
        this.zzc = v;
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final void add(int v, Object object0) {
        double f = (double)(((Double)object0));
        this.zza();
        if(v >= 0) {
            int v1 = this.zzc;
            if(v <= v1) {
                double[] arr_f = this.zzb;
                if(v1 < arr_f.length) {
                    System.arraycopy(arr_f, v, arr_f, v + 1, v1 - v);
                }
                else {
                    double[] arr_f1 = new double[zzje.zzd(arr_f.length)];
                    System.arraycopy(this.zzb, 0, arr_f1, 0, v);
                    System.arraycopy(this.zzb, v, arr_f1, v + 1, this.zzc - v);
                    this.zzb = arr_f1;
                }
                this.zzb[v] = f;
                ++this.zzc;
                ++this.modCount;
                return;
            }
        }
        throw new IndexOutOfBoundsException(this.zze(v));
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final boolean add(Object object0) {
        this.zza(((double)(((Double)object0))));
        return true;
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final boolean addAll(Collection collection0) {
        this.zza();
        zzjv.zza(collection0);
        if(!(collection0 instanceof zzje)) {
            return super.addAll(collection0);
        }
        int v = ((zzje)collection0).zzc;
        if(v == 0) {
            return false;
        }
        int v1 = this.zzc;
        if(0x7FFFFFFF - v1 < v) {
            throw new OutOfMemoryError();
        }
        int v2 = v1 + v;
        double[] arr_f = this.zzb;
        if(v2 > arr_f.length) {
            this.zzb = Arrays.copyOf(arr_f, v2);
        }
        System.arraycopy(((zzje)collection0).zzb, 0, this.zzb, this.zzc, ((zzje)collection0).zzc);
        this.zzc = v2;
        ++this.modCount;
        return true;
    }

    @Override
    public final boolean contains(Object object0) {
        return this.indexOf(object0) != -1;
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final boolean equals(Object object0) {
        if(this == object0) {
            return true;
        }
        if(!(object0 instanceof zzje)) {
            return super.equals(object0);
        }
        if(this.zzc != ((zzje)object0).zzc) {
            return false;
        }
        double[] arr_f = ((zzje)object0).zzb;
        for(int v = 0; v < this.zzc; ++v) {
            if(Double.doubleToLongBits(this.zzb[v]) != Double.doubleToLongBits(arr_f[v])) {
                return false;
            }
        }
        return true;
    }

    @Override
    public final Object get(int v) {
        return this.zzb(v);
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final int hashCode() {
        int v = 1;
        for(int v1 = 0; v1 < this.zzc; ++v1) {
            v = v * 0x1F + zzjv.zza(Double.doubleToLongBits(this.zzb[v1]));
        }
        return v;
    }

    @Override
    public final int indexOf(Object object0) {
        if(!(object0 instanceof Double)) {
            return -1;
        }
        double f = (double)(((Double)object0));
        int v = this.size();
        for(int v1 = 0; v1 < v; ++v1) {
            if(this.zzb[v1] == f) {
                return v1;
            }
        }
        return -1;
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final Object remove(int v) {
        this.zza();
        this.zzf(v);
        double[] arr_f = this.zzb;
        double f = arr_f[v];
        int v1 = this.zzc;
        if(v < v1 - 1) {
            System.arraycopy(arr_f, v + 1, arr_f, v, v1 - v - 1);
        }
        --this.zzc;
        ++this.modCount;
        return f;
    }

    @Override
    protected final void removeRange(int v, int v1) {
        this.zza();
        if(v1 < v) {
            throw new IndexOutOfBoundsException("toIndex < fromIndex");
        }
        System.arraycopy(this.zzb, v1, this.zzb, v, this.zzc - v1);
        this.zzc -= v1 - v;
        ++this.modCount;
    }

    @Override  // com.google.android.gms.internal.measurement.zzie
    public final Object set(int v, Object object0) {
        this.zza();
        this.zzf(v);
        double[] arr_f = this.zzb;
        double f = arr_f[v];
        arr_f[v] = (double)(((Double)object0));
        return f;
    }

    @Override
    public final int size() {
        return this.zzc;
    }

    @Override  // com.google.android.gms.internal.measurement.zzkc
    public final zzkc zza(int v) {
        if(v < this.zzc) {
            throw new IllegalArgumentException();
        }
        return v == 0 ? new zzje(zzje.zza, this.zzc, true) : new zzje(Arrays.copyOf(this.zzb, v), this.zzc, true);
    }

    public final void zza(double f) {
        this.zza();
        double[] arr_f = this.zzb;
        if(this.zzc == arr_f.length) {
            double[] arr_f1 = new double[zzje.zzd(arr_f.length)];
            System.arraycopy(this.zzb, 0, arr_f1, 0, this.zzc);
            this.zzb = arr_f1;
        }
        int v = this.zzc;
        this.zzc = v + 1;
        this.zzb[v] = f;
    }

    public final double zzb(int v) {
        this.zzf(v);
        return this.zzb[v];
    }

    final void zzc(int v) {
        double[] arr_f = this.zzb;
        if(v <= arr_f.length) {
            return;
        }
        if(arr_f.length == 0) {
            this.zzb = new double[Math.max(v, 10)];
            return;
        }
        int v1;
        for(v1 = arr_f.length; v1 < v; v1 = zzje.zzd(v1)) {
        }
        this.zzb = Arrays.copyOf(this.zzb, v1);
    }

    private static int zzd(int v) {
        return Math.max(v * 3 / 2 + 1, 10);
    }

    private final String zze(int v) {
        return "Index:" + v + ", Size:" + this.zzc;
    }

    private final void zzf(int v) {
        if(v < 0 || v >= this.zzc) {
            throw new IndexOutOfBoundsException(this.zze(v));
        }
    }
}

