package com.unity3d.services.ads.gmascar.handlers;

import P2.d;
import com.unity3d.scar.adapter.common.c;
import com.unity3d.scar.adapter.common.i;
import com.unity3d.services.ads.gmascar.utils.GMAEventSender;
import com.unity3d.services.core.misc.EventSubject;

public class ScarRewardedAdHandler extends ScarAdHandlerBase implements i {
    private boolean _hasEarnedReward;

    public ScarRewardedAdHandler(d d0, EventSubject eventSubject0, GMAEventSender gMAEventSender0) {
        super(d0, eventSubject0, gMAEventSender0);
        this._hasEarnedReward = false;
    }

    @Override  // com.unity3d.services.ads.gmascar.handlers.ScarAdHandlerBase, com.unity3d.scar.adapter.common.e
    public void onAdClosed() {
        if(!this._hasEarnedReward) {
            this.onAdSkipped();
        }
        super.onAdClosed();
    }

    @Override  // com.unity3d.scar.adapter.common.i
    public void onAdFailedToShow(int v, String s) {
        this._gmaEventSender.send(c.u, new Object[]{this._scarAdMetadata.c(), this._scarAdMetadata.d(), s, v});
    }

    @Override  // com.unity3d.scar.adapter.common.i
    public void onAdImpression() {
        this._gmaEventSender.send(c.m, new Object[0]);
    }

    @Override  // com.unity3d.scar.adapter.common.i
    public void onAdSkipped() {
        this._gmaEventSender.send(c.C, new Object[0]);
    }

    @Override  // com.unity3d.scar.adapter.common.i
    public void onUserEarnedReward() {
        this._hasEarnedReward = true;
        this._gmaEventSender.send(c.A, new Object[0]);
    }
}

